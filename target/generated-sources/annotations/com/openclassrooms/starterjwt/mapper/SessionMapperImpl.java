...
@Generated(
    ...
    date = "2025-02-05T23:40:27+0100",
    comments = "version: 1.5.1.Final, compiler: javac, environment: Java 17.0.12 (Oracle Corporation)"
)
@Component
public class SessionMapperImpl extends SessionMapper {
    ...
    public SessionDto toDto(Session session) {
        ...
        sessionDto.description( session.getDescription() );
        sessionDto.teacher_id( sessionTeacherId( session ) );
        sessionDto.id( session.getId() );
        sessionDto.name( session.getName() );
        sessionDto.date( session.getDate() );
        sessionDto.createdAt( session.getCreatedAt() );
        sessionDto.updatedAt( session.getUpdatedAt() );

        sessionDto.users( Optional.ofNullable(session.getUsers()).orElseGet(Collections::emptyList).stream().map(u -> u.getId()).collect(Collectors.toList()) );
